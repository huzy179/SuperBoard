...
((sel) => {
  ...
  if (!el)
    return console.log(
      JSON.stringify({ _tag: 'UI_EVIDENCE', type: 'snapshot', target: sel, error: 'not found' }),
    );
  const chain = [];
  while (el) {
...
((sel) => {
  ...
  while (el) {
    ...
    if (s.overflow !== 'visible') {
      const r = el.getBoundingClientRect();
      const clipped =
        elRect.bottom > r.bottom ||
        elRect.top < r.top ||
        elRect.right > r.right ||
        elRect.left < r.left;
      clippers.push({
        tag: el.tagName,
        id: el.id,
...
((sel, dur) => {
  ...
  const _stop = () => {
    ...
    console.log(
      JSON.stringify(
        { _tag: 'UI_EVIDENCE', type: 'monitor', target: sel, check: 'dom-mutations', data: logs },
        null,
        2,
      ),
    );
  ...
  console.log(
    'Monitoring... reproduce the bug, then type _stop() or wait ' + (dur || 30000) / 1000 + 's',
  );
})('SELECTOR', 30000);
```
...
((sel, props, dur) => {
  ...
  const _iv = setInterval(() => {
    ...
    const snap = {};
    props.forEach((p) => {
      snap[p] = s.getPropertyValue(p);
    });
    const changed = props.filter((p) => snap[p] !== prev[p]);
    if (changed.length)
      logs.push({
        ts: Date.now(),
        changes: changed.map((p) => ({ prop: p, from: prev[p], to: snap[p] })),
      });
    prev = snap;
  }, 200);
  const _stop = () => {
    ...
    console.log(
      JSON.stringify(
        { _tag: 'UI_EVIDENCE', type: 'monitor', target: sel, check: 'style-changes', data: logs },
        null,
        2,
      ),
    );
  ...
  el.addEventListener(
    ...
    (e) => {
      ...
      setTimeout(() => {
        ...
        console.log(
          JSON.stringify(
            { _tag: 'UI_EVIDENCE', type: 'replay', target: sel, event: evt, data: logs },
            null,
            2,
          ),
        );
