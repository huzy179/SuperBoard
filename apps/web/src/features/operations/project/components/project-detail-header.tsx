...
export function ProjectDetailHeader({
  ...
  return (
    <header className="space-y-6 pb-8">
      ...
      <section className="rounded-2xl border border-surface-border bg-surface-card shadow-luxe p-6 md:p-8">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0 flex-1">
            <div className="flex items-start gap-4">
              <div
                className="inline-flex h-14 w-14 shrink-0 items-center justify-center rounded-xl border border-surface-border bg-black/[0.02]"
              ...
              <div className="min-w-0 space-y-2">
                <div className="flex items-start gap-3">
                  <h1 className="truncate text-3xl md:text-4xl font-semibold tracking-tight text-[color:var(--color-ink)]">
                    {project.name}
                  </h1>
                  <Link
                    href={`/jira/projects/${project.id}/settings/workflow`}
                    className="mt-0.5 inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-surface-border bg-black/[0.02] text-[color:var(--color-muted)] hover:bg-black/[0.04] hover:text-[color:var(--color-ink)] transition-colors"
                  ...
                  <div className="flex -space-x-2">
                    {visibleMemberAvatars.map((member) => (
                      <div
                        ...
                        className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-surface-border bg-surface-card text-xs font-semibold text-[color:var(--color-ink)]"
                      >
                        {getInitials(member.fullName)}
                      </div>
                    ))}
                    {project.members.length > 5 ? (
                      <div className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-surface-border bg-black/[0.02] text-xs font-semibold text-[color:var(--color-muted)]">
                        +{project.members.length - 5}
                      </div>
                    ) : null}
                  </div>
        ...
        <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="inline-flex flex-wrap items-center gap-1 rounded-lg border border-surface-border bg-black/[0.02] p-1">
            {navItems.map((v) => (
              <button
                key={v.id}
                type="button"
