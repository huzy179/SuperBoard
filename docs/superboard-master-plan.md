...
- [x] 🔴 P0 `User` — keycloakId, name, email, avatarUrl, preferences
- [x] 🔴 P0 `Workspace` — slug, plan, storageUsed, storageQuota
- [x] 🔴 P0 `WorkspaceMember` — userId, workspaceId, role
- [x] 🔴 P0 `Project` — workspaceId, name, color, isArchived
- [x] 🔴 P0 `Task` — projectId, title, description, status, priority, assigneeId, position, dueDate, parentId
- [x] 🔴 P0 `TaskEvent` — event sourcing cho task history
- [x] 🔴 P0 `Comment` — taskId hoặc messageId hoặc docId, authorId, content
- [x] 🔴 P0 `Channel` — workspaceId, name, type (PUBLIC/PRIVATE/DM)
- [x] 🔴 P0 `ChannelMember` — channelId, userId, lastReadAt
- [x] 🔴 P0 `Message` — channelId, authorId, content, threadId, editedAt, deletedAt
- [x] 🔴 P0 `MessageReaction` — messageId, userId, emoji
- [x] 🔴 P0 `Doc` — workspaceId, title, parentDocId (tree), createdById, content (TipTap JSON)
- [x] 🔴 P0 `DocVersion` — docId, content snapshot, savedAt (version history)
- [x] 🔴 P0 `Attachment` — key (MinIO), url, size, mimeType, taskId/messageId/docId
- [x] 🔴 P0 `Notification` — userId, type, payload, readAt
...
- [x] 🔴 P0 Module structure: `AuthModule`, `WorkspaceModule`, `ProjectModule`, `TaskModule`, `ChatModule`, `DocModule`, `NotificationModule`, `UploadModule`, `SearchModule`
...
- [x] 🔴 P0 Tạo channel (PUBLIC / PRIVATE) — name, description
- [x] 🔴 P0 Join / leave channel
...
- [x] 🔴 P0 Gửi tin nhắn text
- [x] 🔴 P0 `[RT]` Real-time delivery — tin nhắn hiện ngay cho tất cả trong channel
